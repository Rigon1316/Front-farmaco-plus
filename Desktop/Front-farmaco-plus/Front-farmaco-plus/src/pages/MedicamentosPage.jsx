...
function MedicamentosPage() {
  ...
  const buscarMedicamentoPorNombre = async (nombre) => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 404) {
          setError("No se encontró un medicamento con ese nombre");
          setMedicamentos([]);
        } else {
          throw new Error(`Error ${res.status}: ${res.statusText}`);
        }
      } else {
        const medicamentosEncontrados = await res.json();
        setMedicamentos(
          Array.isArray(medicamentosEncontrados)
            ? medicamentosEncontrados
            : [medicamentosEncontrados]
        );
      }
    } catch (err) {
      console.error("Error buscando medicamento por nombre:", err);
  ...
  const handleBusquedaNombre = (e) => {
    e.preventDefault();
    buscarMedicamentoPorNombre(busquedaNombre);
  };
