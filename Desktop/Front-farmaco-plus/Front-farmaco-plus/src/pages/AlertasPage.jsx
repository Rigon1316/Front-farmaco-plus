...
function DetalleVentaPage() {
  ...
  // Estado local para IDs de alertas leídas con persistencia
  const [leidas, setLeidas] = useState(() => {
    // Cargar alertas leídas desde localStorage al inicializar
    const savedLeidas = localStorage.getItem("alertasLeidas");
    return savedLeidas ? JSON.parse(savedLeidas) : [];
  });
  ...
  // Guardar alertas leídas en localStorage cuando cambien
  useEffect(() => {
    localStorage.setItem("alertasLeidas", JSON.stringify(leidas));
  }, [leidas]);
  ...
  // Función para limpiar todas las alertas leídas (opcional)
  const limpiarAlertasLeidas = () => {
    if (
      window.confirm(
        "¿Seguro que deseas marcar todas las alertas como no leídas?"
      )
    ) {
      setLeidas([]);
  ...
  const marcarComoLeida = (id) => {
    if (!leidas.includes(id)) {
      setLeidas((prev) => {
        const newLeidas = [...prev, id];
        // Guardar inmediatamente en localStorage
        localStorage.setItem("alertasLeidas", JSON.stringify(newLeidas));
        return newLeidas;
      });
    }
  };
    ...
    >
      ...
      <div style={{ width: "100%", maxWidth: 1400 }}>
        ...
        >
          ...
          <div>{/* El botón Nueva Alerta ha sido eliminado */}</div>
        </div>
